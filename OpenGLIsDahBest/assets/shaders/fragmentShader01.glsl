...
uniform bool enableTextureMovement;
...
if (bIsHeightMap)
{
    vec4 heightMapColor = texture(heightMapTexture, fUV);
    finalOutputColour.rgb = heightMapColor.rgb;
...
{
    vec4 secondMapColor = texture(secondTexture, fUV);
    finalOutputColour.rgb = secondMapColor.rgb;
    finalOutputColour.w = alphaTransparency;
    return;
}
...
if (bIsSkyBoxObject)
{
    ...
    vec3 skyBoxPixelColour_RGB = mix(texture(skyBoxTexture, theNormal), texture(skyBoxTexture2, theNormal), blendFactor).rgb;
    finalOutputColour.rgb = skyBoxPixelColour_RGB;
...
finalOutputColour.rgb = calcualteLightContrib(finalText_RGB.rgb,
                                              fNormal.xyz,
                                              fVertexPosWorld.xyz,
                                              specularColourRGB_Power).rgb;
